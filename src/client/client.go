...
var T = flag.Int("T", 20, "Number of threads (simulated clients).")
var s = flag.Float64("s", 2, "Zipfian s parameter")
var v = flag.Float64("v", 1, "Zipfian v parameter")
var thinktime *int = flag.Int("think", 100, "time in nanoseconds for thinking")
var zKeys = flag.Uint64("z", 1000, "Number of unique keys in zipfian distribution.")
...
func clientWriter(idx int, writerList []*bufio.Writer, stop chan int, next chan int, wg *sync.WaitGroup) {
	...
	for id := int32(0); int(id) < *reqsNb; id++ {
		select {
		case i := <-stop:
		...
		default:
			...
			// for {
			zipfGen := zipfian.NewZipfianGenerator(*zKeys, *theta)
			sid := rand.Int() % N
			...
			if r < *conflicts {
				// args.Command.K = 42
				// add zipfian way of assigning K instead of 42
				args.Command.K = state.Key(zipfGen.NextNumber())
			} else {
				args.Command.K = state.Key(r)
			}*/
			// Determine key
			keyGenerated := state.Key(zipfGen.NextNumber())
			log.Println("KEY GENERATED = ", keyGenerated)
			args.Command.K = keyGenerated
